...
from .forms import UserRegisterForm, SellerProfileForm  # <-- Fix here
from .models import SellerProfile, Product

def seller_register(request):
    if request.method == 'POST':
        user_form = UserRegisterForm(request.POST)
        seller_form = SellerProfileForm(request.POST)  # <-- Fix here
        ...
        seller_form = SellerProfileForm()  # <-- Fix here
